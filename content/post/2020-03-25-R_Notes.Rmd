...
where $X$ is 1706 X 4 matrix because there are 1706 observations (rows) and 4 variables. We can use the built in function `lm()` to run a linear model with life expectancy as our dependent variable. First, convert the population variable into a logged value.
```{r}
gapminder$pop_log <- log(gapminder$pop) # make a logged population variable
```
Then use estimate the linear model
